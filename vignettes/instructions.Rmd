...
Your working folder may also contain **the following two things**, to work with the [save_history()] function.
...
A `history.csv` file will be created the first time you call `save_history()`, with the 8-number summary and module effects for the year in `year.txt`.
...
Then, when `save_history()` is called in that yearm the data will be appended.
