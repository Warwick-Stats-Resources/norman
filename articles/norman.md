...
The installation of **norman** should also trigger the installation of
five other packages if you do not have them already. They are
**aplpack**, **ggplot2**, **dplyr**, **readr**, and **scales**.
...
If a grouping is desired for the scatter plots on the module pages, the
following **two files** must **both** be in the working folder.

#### 3d. A file linking each student with their course

This must be a file named `student_course.csv`, with two columns, `ID`
and `Course`.

`ID` is the 7-digit student ID number. `Course` is their course code,
e.g. “USTA-G302”.

#### 3e. A file linking course codes with the groupings for the scatter plots

This must be a file named `course_mappings.csv`, with two columns,
`Course` and `Group`.

`Course` is a course code, e.g. “USTA-G302”. There should be one course
code for each (unique) `Course` in `student_course.csv`. `Group` is
whatever grouping is needed for the scatter plots of module marks
against the median of ST modules taken, as it appears on each individual
module page of the report. This will likely be degree types, e.g. “BSc”,
“IM”.

### For saving history

If you wish to save the marks summaries and module effects with the
\[save_history()\] function, the working folder *must* contain a file
with the year and *may* already contain the history. Note that neither
of these files are needed to knit the report.
...
If there is no file called `history.csv` in the working folder, a
`history.csv` file will be created the first time you call
...
[`save_history()`](https://warwick-stats-resources.github.io/norman/reference/save_history.md)
is called in that year, the data will be appended.
...
Check there is a `year.txt` file in the working folder (see point 3f
above) and (optionally) a `history.csv` file (see point 3g).
...
Ask Jon Forster, Ella Kaye or David Firth for advice (in that order!),
if any of the above is unclear or does not work.
