...
```{r}
has_groups <- file.exists("../student_course.csv") && file.exists("../course_mappings.csv")

if (has_groups) {
    student_course <- readr::read_csv("../student_course.csv") |>
      dplyr::mutate(ID = as.character(ID))
    course_mappings <- readr::read_csv("../course_mappings.csv")

    if (!identical(colnames(student_course), c("ID", "Course"))) {
      stop("student_course.csv must have column names 'ID' and 'Course'")
    }

    if (!identical(colnames(course_mappings), c("Course", "Group"))) {
      stop("course_mappings.csv must have column names 'Course' and 'Group'")
    }
}
```

```{r scatter-module-code, fig.width=5, fig.height=7, out.height='5in', fig.align='right'}
norman::scatter("module-code", marks_matrix, student_overall_median, has_groups)
```
